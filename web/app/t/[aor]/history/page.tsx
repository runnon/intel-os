    ...
    <main className="flex-1 overflow-y-auto">
      <div className="max-w-3xl mx-auto px-6 py-8">
        <header className="mb-6 flex items-baseline gap-4">
          <Link href={`/t/${slug}`} className="font-mono text-sm tracking-[0.25em] text-[#8b949e] hover:text-white">
            ◂ {aor}
          </Link>
          <h1 className="font-mono text-lg tracking-widest">ISSUE ARCHIVE</h1>
        </header>
        <p className="text-xs text-[#8b949e] mb-6 max-w-xl">
        ...
        <table className="w-full font-mono text-xs">
          <thead>
            <tr className="text-[#6e7681] text-left border-b border-[#30363d]">
          ...
          <tbody>
            {(data ?? []).map((i) => (
              <tr key={i.serial} className="border-b border-[#21262d] hover:bg-[#161b22]">
                <td className="py-2 pr-4">
                  <Link href={`/i/${i.serial}`} className="text-[#4493f8] hover:underline">
                    {i.serial}
                  </Link>
                </td>
                <td className="py-2 pr-4 text-[#8b949e]">{zulu(i.info_cutoff)}</td>
